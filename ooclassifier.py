...
Debug = True  # Sometimes, print for debugging.  Overridable on command line.
...
class TrainingInstance(C274):
    ...
    def preprocess_words(self, mode=''):
        ...
        def Num():
            """
            Remove all numbers UNLESS the token consists only of numbers.
            :return:
            """
            ...
            for i in range(len(Array)):
                ...
                # Adding a token to array consisting of number only
                try:
                    int(Array[i])

                # Dealing with string of number & other characters
                except ValueError:
                    MWord = str()
                    for letter in Array[i]:
                        if letter not in Num_Array:
                            MWord += letter
                    Array[i] = MWord

        def Stopwords():
            """
            Removes the stopwords from the array
            :return:
            """
            # List of Stopwords
            SWords = ["i", "me", "my", "myself", "we", "our", "ours",
        ...
        def Output():
            ...
            # Checking the state & number of argument
        ...
        # If number of arguments are 2
            ...
            # If 2nd  argument is keep-digits
            if mode == "keep-digits":
                a = input()
                Array = a.split()
                Lowercase()
            ...
            # If 2nd  argument is keep-symbols
            ...
            # If 2nd  argument is anything else
            else:
                print('Invalid input. python3 preprocess .py <mode >')
                sys.exit()

        # If number of arguments are more than 2
        else:
            print('Invalid input. python3 preprocess .py <mode >')
            sys.exit()

    # If initial argument is not preprocess.py
    else:
    print('Invalid input. python3 preprocess .py <mode >')
